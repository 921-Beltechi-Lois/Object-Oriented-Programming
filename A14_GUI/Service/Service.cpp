...
// gui
std::vector<Event> Service::get_events_by_string(const std::string &filter_string) {
    std::vector<Event> all_events=this->repo.getAllElems_Repo();
    //int count = (int) std::count_if(all_events.begin(), all_events.end(), [=](Event e) { return e.ToString().find(filter_string) != std::string::npos;});
    std::vector<Event> filter_event;
    std::copy_if(all_events.begin(), all_events.end(), std::back_inserter(filter_event), [=](Event& e){ return e.ToString().find(filter_string) != std::string::npos;});
    std::sort(filter_event.begin(), filter_event.end(), [](Event& e1, Event& e2){ return e1.getTitle() < e2.getTitle();});
